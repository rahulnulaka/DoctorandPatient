...
import com.Connecting.Doctors.Patients.Connecting.Doctors.Patients.Exceptions.*;
...
public class PatientService {
    ...
    public String deletePatient(Integer patientId) throws Exception{
        Optional<Patient> optionalPatient=patientRepository.findById(patientId);
        if(optionalPatient.isEmpty()){
            throw new PatientNotFound("Patient is not available");
        }
        Patient patient=optionalPatient.get();

        patientRepository.deleteById(patientId);
        return "Successfully removed patient from DataBase";
    }
}
